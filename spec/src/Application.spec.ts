...
import { applicationError, delay, error, fakePathToApp, runApp, types } from "./test-utils.js";
...
describe("Application", () => {
	...
	it("should not allow required positional arguments to follow optional ones", () => {
		...
		expect(() => {
			app.command("cmd2").description("test command").args({
				positionalArgs: [
					{
						name: "arg1",
						description: "test arg",
	...
	it("should run the default command if no subcommand is specified", async () => {
		const app = new Application("test-app", "Autogenerated test app");
		app.command("cmd1").description("test command").aliases("alias1").args({}).impl(() => {});
		app.command("cmd2").default().description("test command 2").aliases("alias2").args({}).impl(() => {});
		spyOn(app.commands["cmd2"]!, "handler").and.callThrough();
		await runApp(app, []);
		expect(app.commands["cmd2"]!.handler).toHaveBeenCalled();
	});
	...
	it("should set process.exitCode when an exit code is returned", async () => {
		const app = new Application("test-app", "Autogenerated test app");
		let code = 0;
		...
		code = 2;
		await expectAsync(app.run(["node", fakePathToApp, "cmd1"], {throwOnError: true, setProcessExitCodeOnHandlerReturn: false}))
			.toBeRejectedWith(error(`${code}`));
	});
	...
	it("should run the help command when called with --help", async () => {
		const app = new Application("test-app", "Autogenerated test app");
		app.command("cmd1").description("test command").args({
			namedArgs: {
				namedarg1: arg()
		...
		spyOn(app.commands["help"]!, "handler");
		await runApp(app, [`cmd1`, `--help`]);
		expect(app.commands["cmd1"]!.handler).not.toHaveBeenCalled();
		expect(app.commands["help"]!.handler).toHaveBeenCalled();
	});

	it("should not run the help command when called with --help if configured that way", async () => {
		const app = new Application("test-app", "Autogenerated test app");
		app.command("cmd1").description("test command").args({
			namedArgs: {
				namedarg1: arg().optional()
		...
		await runApp(app, [`cmd1`, `--help`]);
		expect(app.commands["cmd1"]!.handler).toHaveBeenCalled();
		expect(app.commands["help"]!.handler).not.toHaveBeenCalled();
	});

	it("should run commands under subcategories", async () => {
		const app = new Application("test-app", "Autogenerated test app");
		app.category("cat1", "desc1", cat => {
			cat.command("cmd1").description("test command").args({
				namedArgs: {
					namedarg1: arg()
				}
			}).impl((opts, app) => {
		...
		spyOn(app.commands["cat1"]!.subcategoryApp!.commands["cmd1"]!, "handler").and.callThrough();
		await runApp(app, [`cat1`, `cmd1`, `--namedarg1`, `namedvalue1`]);
		expect(app.commands["cat1"]!.subcategoryApp!.commands["cmd1"]!.handler).toHaveBeenCalled();
	});

	it("should run the help command under two subcategories", async () => {
		spyOn(Application.prototype, "runHelpCommand" as never);
		const app = new Application("test-app", "Autogenerated test app");
		app.category("cat1", "desc1", app => {
			app.category("cat2", "desc1", app => {
				app.command("cmd1").description("test command").args({
					namedArgs: {
						namedarg1: arg()
					}
		...
		await runApp(app, [`help`, `cat1`, `cat2`, `cmd1`]);
		await runApp(app, [`cat1`, `help`, `cat2`, `cmd1`]);
		await runApp(app, [`cat1`, `cat2`, `help`, `cmd1`]);
		await runApp(app, [`cat1`, `cat2`, `cmd1`, `--help`]);
		expect(Application.prototype["runHelpCommand"]).toHaveBeenCalledTimes(4);
	});

	it("should set process.exitCode when an exit code is returned asynchronously", async () => {
		const app = new Application("test-app", "Autogenerated test app");
		let code = 0;
		app.command("cmd1").description("test command").args({}).impl(async (opts, app) => {
			await delay(2);
		...
		await app.run(["node", fakePathToApp, "cmd1"], {throwOnError: true, setProcessExitCodeOnHandlerReturn: true});
		expect(process.exitCode).toBe(code);
		...
		await expectAsync(app.run(["node", fakePathToApp, "cmd1"], {throwOnError: true, setProcessExitCodeOnHandlerReturn: false}))
			.toBeRejectedWith(error(`${code}`));
		expect(process.exitCode).not.toBe(code);
	});

	it("should set the command as default when using onlyCommand", async () => {
		const app = new Application("test-app", "Autogenerated test app");
		app.onlyCommand().args({
			namedArgs: {
				namedarg1: arg().optional(),
			}
		}).impl(() => {});

		spyOn(app.commands["test-app"]!, "handler");
		await runApp(app, []);
		expect(app.commands["test-app"]!.handler).toHaveBeenCalled();
	});
	it("should adjust help messages when using onlyCommand", async () => {
		const appName = "test-app";
		const appDesc = "Autogenerated test app";
		const app = new Application(appName, appDesc);
		const argName = "namedarg1";
		const argDesc = "Named arg 1 description";
		app.onlyCommand().args({
			namedArgs: {
				[argName]: arg().optional()
					.description(argDesc),
			}
		}).impl(() => {});

		spyOn(app.commands[appName]!, "handler");
		spyOn(process.stdout, "write");
		await runApp(app, ["help"]);
		expect(app.commands[appName]!.handler).not.toHaveBeenCalled();
		// eslint-disable-next-line @typescript-eslint/unbound-method
		expect(process.stdout.write).toHaveBeenCalledOnceWith(`
Help for command ${appName}:
${appDesc}
Usage: ${appName} [--${argName} <${argName}>]
